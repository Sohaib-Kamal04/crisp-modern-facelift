...
const Hero = () => {
  ...
  // Handle Scroll for the rounded corner animation
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    // Outer container provides the background color (white) that shows behind the rounded corners
    <div className="bg-background relative">
      <section
        className="relative min-h-screen flex items-center justify-center overflow-hidden transition-all duration-100 ease-out will-change-transform"
        style={{
          // As you scroll (progress 0 -> 1), the radius goes 0px -> 60px
          borderBottomLeftRadius: `${scrollProgress * 60}px`,
          borderBottomRightRadius: `${scrollProgress * 60}px`,
          // Optional: Subtle scale effect to enhance the "card" feel
      ...
      >
        ...
        <div className="absolute inset-0 z-0">
          ...
          {/* 1. BRIGHTER OVERLAY: Reduced opacity from 60 to 30 */}
          <div className="absolute inset-0 bg-black/30" />
          
          {/* 2. SOFT TRANSITION: Bottom Gradient Fade */}
          <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-background via-background/20 to-transparent z-10" />
        </div>
